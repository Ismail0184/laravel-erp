            ...
            <div class="col-lg-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <hr>
                        @if($vouchermaster->status!=='DELETED')
                            <h1 class="text-center text-uppercase">Cheque Payment Voucher</h1>
                        @else
                            <h1 class="text-center text-uppercase"><del>Cheque Payment Voucher</del></h1>
                        @endif
                        <div class="row">
                            ...
                            <table class="table table-nowrap">
                                ...
                                @foreach($cpayments as $cpayment)
                                    <tr>
                                        ...
                                        <td>{{$cpayment->ledgerforvoucher->ledger_name}}</td>
                                        <td>{{$cpayment->narration}}</td>
                                        <td class="text-right">{{number_format($cpayment->dr_amt,2)}}</td>
                                        <td class="text-right">{{number_format($cpayment->cr_amt,2)}}</td>
                                    </tr>
                                    @php($dr_total = $dr_total +$cpayment->dr_amt )
                                    @php($cr_total = $cr_total +$cpayment->cr_amt )
                                @endforeach
                                <tr>
                        ...
                        <div class="d-print-none">
                            <div class="float-left">
                                @if($vouchermaster->status!=='DELETED')
                                    ...
                                    <a href="{{route('acc.voucher.payment.download',['voucher_no' => $vouchermaster->voucher_no])}}" class="btn btn-primary waves-effect waves-light mr-1"><i class="fa fa-download"></i></a>
                                @endif
                            </div>
