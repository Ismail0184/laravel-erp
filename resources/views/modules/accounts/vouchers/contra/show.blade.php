                    ...
                    <div class="card-body">
                        <div class="invoice-title">
                            <h1 class="font-size-20 text-center">{{ Auth::user()->getCompanyInfo->company_name }}<br>
                                <small class="font-size-10">{{ Auth::user()->getCompanyInfo->address }}</small>
                            </h1>
                        </div>
                        <hr>
                        @if($vouchermaster->status!=='DELETED')
                            <h1 class="text-center text-uppercase">Contra Voucher</h1>
                        @else
                            <h1 class="text-center text-uppercase"><del>Contra Voucher</del></h1>
                        @endif
                        <div class="row">
                        ...
                        <div class="table-responsive">
                            <table class="table table-nowrap">
                                ...
                                @foreach($contras as $contra)
                                    ...
                                    @php($dr_total = $dr_total +$contra->dr_amt )
                        ...
                        <div class="d-print-none">
                            <div class="float-left">
                                @if($vouchermaster->status!=='DELETED')
                                    ...
                                    <a href="{{route('acc.voucher.contra.download',['voucher_no' => $vouchermaster->voucher_no])}}" class="btn btn-primary waves-effect waves-light mr-1"><i class="fa fa-download"></i></a>
                                @endif
                            </div>
                            <form action="{{route('acc.voucher.contra.status.update', ['voucher_no'=>$vouchermaster->voucher_no])}}" method="post">
                                @csrf
