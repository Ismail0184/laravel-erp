    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                ...
                <form method="POST" action="@if(request('id')>0) {{route('hrm.setup.grade.update', ['id'=>$grade->id])}} @else {{route('hrm.setup.grade.store')}} @endif">
                    @csrf
                    <input type="hidden" name="entry_by" value="{{ Auth::user()->id }}">
                    <div class="form-group row mb-2">
                        <label for="horizontal-email-input" class="col-sm-3 col-form-label">Grade <span class="required text-danger">*</span></label>
                        <div class="col-sm-9">
                            <input type="text" name="grade" @if(request('id')>0) value="{{$grade->grade}}" @endif class="form-control" required>
                        </div>
                    </div>
                    @if(request('id')>0)
