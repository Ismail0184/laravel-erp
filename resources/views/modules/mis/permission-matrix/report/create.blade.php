    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                ...
                @endif
                <form method="POST" action="{{route('mis.permissionMatrix.reports.store')}}" enctype="multipart/form-data">
                    @csrf
                        ...
                        <div class="col-sm-9">
                            <select class="form-control select2" name="company_id" required="required">
                                ...
                                @foreach($companies as $company)
                                    <option value="{{$company->company_id}}" >{{$company->company_id}} : {{$company->getComData->company_name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-9">
                            <select class="form-control select2" name="module_id" required="required">
                                ...
                                @foreach($modules as $module)
                                    <option value="{{$module->module_id}}" >{{$module->module_id}} : {{$module->getModuleData->modulename}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="form-group row mb-4">
                        <label for="horizontal-email-input" class="col-sm-3 col-form-label">Reports<span class="required text-danger">*</span></label>
                        <div class="col-sm-9">
                            <select class="form-control select2" multiple name="report_id[]" required="required">

                                @foreach($reports as $report)
                                    <option value="{{$report->report_id}}" >{{$report->report_id}} : {{$report->report_name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group row justify-content-end">
                        <div class="col-sm-9">
                            <div>
                                <a class="btn btn-danger" href="{{route('mis.permissionMatrix.reports')}}">Cancel</a>
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">Permitted Reports</h4>
                        ...
                        <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                            ...
                            <tbody>
                            @foreach($userReportPermissions as $user)
                                <tr>
                                    <td style="text-align: center; vertical-align: middle">{{$loop->iteration}}</td>
                                    <td style="text-align:left; vertical-align: middle">{{$user->report_id}} : {{$user->getReportInfo->report_name}}</td>
                                    <td style="text-align:left; vertical-align: middle">{{$user->company_id}} : {{$user->getReportLevelInfo->optgroup_label_name ?? '-' }}</td>
                                    ...
                                    <td class="text-center" style="vertical-align: middle">
                                        <form action="{{route('mis.permissionMatrix.reports.update', ['id' => $user->id])}}" method="post">
                                            @csrf
                                            <input type="hidden" name="user_id" value="{{ request('id') }}">
